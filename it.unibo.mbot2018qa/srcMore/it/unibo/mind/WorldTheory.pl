...
timeForward( 255).
timeTurn( 500).
init:-assign( nstep,0),assign( curIterExplore,0),assign( nbranch,1).
goForward( N,T):-getVal( nstep,N),timeForward( T),assign( repeatForward,0),output( goForward( N,T)).
execForward( T):-timeForward( T),getVal( repeatForward,R),getVal( nstep,N),output( execForward( R,N,T)),eval( lt,R,N),inc( repeatForward,1,_).
checkAllBranch( T):-timeForward( T),inc( nbranch,1,N),output( checkAllBranch( N,V,T)),eval( lt,N,5).
checkEndExplore( MAX):-getVal( curIterExplore,V),inc( nstep,1,N),output( checkEndExplore( V,MAX,N)),eval( gt,V,MAX).
/*
------------------------------------------------------------------------
