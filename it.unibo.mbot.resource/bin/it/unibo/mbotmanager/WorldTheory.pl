%==============================================
% WorldTheory.pl for actor mbotmanager
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatumbotmanager).	%%old version (deprecated)
actorobj(qatumbotmanager).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor mbotmanager
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of mbotmanager" ). 
...
%-------------------------------------------------
%  User static rules about mbotmanager
%------------------------------------------------- 
initResourceState:-assign( tokenVal,10),assign( stateOfRobot,free).
newAccessToken( V1):-inc( tokenVal,10,V1),actorPrintln( newAccessToken( V1)),assign( tokenVal,V1).
engage( token( TOKEN)):-getVal( stateOfRobot,free), ! ,newAccessToken( TOKEN),actorPrintln( engageeeeeeeeeeeeeeeee( TOKEN)),assign( stateOfRobot,busy( TOKEN)).
engage( failure).
checkToken( TOKEN):-actorPrintln( checkToken( TOKEN)),getVal( stateOfRobot,busy( TOKEN)).
isengaged( TOKEN):-getVal( stateOfRobot,busy( TOKEN)).
disengage:-actorPrintln( dddddddddddddddddisengage),assign( stateOfRobot,free).
/*
------------------------------------------------------------------------
