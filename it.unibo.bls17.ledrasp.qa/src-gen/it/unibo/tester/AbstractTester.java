...
public abstract class AbstractTester extends QActor { 
	...
	protected IActorAction action;
		...
		*/    
	    ...
	    StateFun init = () -> {	
	    try{	
	     PlanRepeat pr = PlanRepeat.setUp(getName()+"_init",10);
	     pr.incNumIter(); 	
	    	String myselfName = "init";  
	    	temporaryStr = "tester(starts)";
	    	println( temporaryStr );  
	    	//delay  ( no more reactive within a plan)
	    	aar = delayReactive(2000,"" , "");
	    	if( aar.getInterrupted() ) curPlanInExec   = "init";
	    	if( ! aar.getGoon() ) return ;
	    	temporaryStr = QActorUtils.unifyMsgContent(pengine, "ctrlEvent(CATEG,NAME,CMD)","ctrlEvent(leds,led1,on)", guardVars ).toString();
	    	emit( "ctrlEvent", temporaryStr );
	    	//delay  ( no more reactive within a plan)
	    	aar = delayReactive(1000,"" , "");
	    	if( aar.getInterrupted() ) curPlanInExec   = "init";
	    	if( ! aar.getGoon() ) return ;
	    	temporaryStr = QActorUtils.unifyMsgContent(pengine, "ctrlEvent(CATEG,NAME,CMD)","ctrlEvent(leds,led1,off)", guardVars ).toString();
	    	emit( "ctrlEvent", temporaryStr );
	    	//delay  ( no more reactive within a plan)
	    	aar = delayReactive(1000,"" , "");
	    	if( aar.getInterrupted() ) curPlanInExec   = "init";
	     ...
	     connectToMqttServer("tcp://192.168.137.1:1883");
	    	repeatPlanNoTransition(pr,myselfName,"tester_"+myselfName,true,false);
