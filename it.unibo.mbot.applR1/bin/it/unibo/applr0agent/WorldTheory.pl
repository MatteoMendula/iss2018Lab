%==============================================
% WorldTheory.pl for actor applr0agent
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatuapplr0agent).	%%old version (deprecated)
actorobj(qatuapplr0agent).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor applr0agent
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of applr0agent" ). 
...
robotFinalPosition( RES):-sonar1ok( D1),actorPrintln( sonar1ok( D1)), ! ,retract( sonar2ok( D2)),actorPrintln( sonar2ok( D2)),eval( minus,D2,D1,DELTA),finalPosVal( DELTA,RES).
robotFinalPosition( wrong).
finalPosVal( DELTA,wrong):-eval( gt,DELTA,3), ! .
