%==============================================
% WorldTheory.pl for actor buttonhl
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatubuttonhl).	%%old version (deprecated)
actorobj(qatubuttonhl).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor buttonhl
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of buttonhl" ). 
...
%-------------------------------------------------
%  User static rules about buttonhl
%------------------------------------------------- 
config( gui).
mqttServer( "tcp://m2m.eclipse.org:1883").
topicSend( "unibo/mqtt/bls").
config( MQTTSERV,none,TSEND):-mqttServer( MQTTSERV),topicSend( TSEND).
dataToPublish( TSEND,sensordata( CONTENT)):-topicSend( TSEND),retract( msg( MSGID,"event",SENDER,none,CONTENT,SEQNUM)), ! .
